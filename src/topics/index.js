...
Topics.getTopicsByTitleKeywords = async function (
	...
	// allow ACP setting to override editDistancePercentAllowed when not explicitly passed
	if (typeof editDistancePercentAllowed === 'undefined' || editDistancePercentAllowed === null) {
		try {
			const searchSettings = await meta.settings.get('search') || {};
			const cfg = parseFloat(searchSettings.fuzzySensitivity);
			if (!Number.isNaN(cfg) && cfg > 0) {
				// fuzzySensitivity stored as percent (e.g. 20 => 20%)
				editDistancePercentAllowed = Math.max(0, Math.min(1, cfg / 100));
			}
		} catch (e) {
	...
	for (const topic of topicsData) {
		if (topic && topic.tid && topic.title) {
			...
			const fuzzyMatchFunction = (keyword) => {
				...
				const threshold = Math.floor(keyword.length * editDistancePercentAllowed);
				
				// Check variable-length substrings to handle insertions/deletions
				// Search substrings that are ±2 chars from keyword length
				const minLen = Math.max(1, keyword.length - 2);
				const maxLen = keyword.length + 2;
				
				for (let len = minLen; len <= maxLen; len++) {
					for (let i = 0; i <= title.length - len; i++) {
						const substring = title.substring(i, i + len);
						if (calculateEditDistance(substring, keyword) <= threshold) {
							return true;
						}
					}
				}
